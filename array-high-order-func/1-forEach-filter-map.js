...
// forEach() : 배열의 반복문을 깔끔하게 처리, 리턴이 없음
numbers.forEach((n, i) => {
    if (i % 2 === 0) {
        console.log(n);
    }
...
console.log(f1);
console.log(numbers); // 원본은 안바뀜

console.log('===================');

// forEach()랑 다른 점: 리턴이 있다.
...
const m1 = numbers.map((n, idx) => {
    console.log(n, idx); // 6칸 모두 undefined
    // return 100; // 6칸 모두 100 들어감
    // return n++; // ++ 뒤에 붙이면 배열에 넣고 나서 증가, ++ 앞에 붙이면 증가하고나서 배열에 넣음
    return n + 300; //
});

console.log(m1);

// const m2 = foods.map(foods => food[0]); ==> 각 음식의 첫글자들만 저장
...
console.log(m3);
// 이게 만약 필터였으면 찌개 들어간 2개만 출력
// 맵이어서 [ false, true, false, false, true ]
...
*/
// 공교롭게도 프로퍼티 이름이랑 변수이름이랑 같을 수 있음
// 이럴 경우 자바스크립트는 한번만 써도 허용됨 (변수명을 그냥 키로 설정해줌)
...
/*const finalUserList2 = userList
    .filter(user => user.salary >= 3000000 && user.age >= 30 && user.age < 40)
    // 맵 옆에 있는 user에 alt + enter=> 구조분해할당선언
    // 다 지우고 한줄로 만들기, 별칭도 가능
    .map(user => ({
